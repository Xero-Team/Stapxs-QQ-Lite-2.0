...
- The system-notice long-press path now provides a complete `MessageContact` shape rather than an `any` assertion. Renderer typecheck passes; remaining explicit application `any` is limited to Chat, Info, terminal, and AI stream compatibility surfaces.
