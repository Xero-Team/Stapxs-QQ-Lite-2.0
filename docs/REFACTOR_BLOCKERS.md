...
- The system-notice long-press path now provides a complete `MessageContact` shape rather than an `any` assertion. The Info view now uses explicit chat/member/bulletin shapes and unknown-safe member action boundaries, removing its application-level `any` usage. Renderer typecheck, build, contract tests, and release metadata checks pass; remaining explicit application `any` is concentrated in Chat, terminal, and AI stream compatibility surfaces.
