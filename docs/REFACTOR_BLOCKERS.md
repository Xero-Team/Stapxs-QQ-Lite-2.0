...
- Composer caches now use the dedicated `MessageSegmentElem` type in Chat, command-line, danmaku, and sender parsing paths; inbound `MsgItemElem` remains a separate legacy event model while its dynamic OneBot fields are migrated.
- Web, Electron, Tauri, and Android build jobs now emit per-job `SHA256SUMS`; the release job verifies its combined checksum manifest before publishing. Web builds also publish `build-metrics.json` with artifact count and raw/gzip sizes. Native runtime startup, memory, and signed artifact evidence still require the corresponding hosted platform environments.
