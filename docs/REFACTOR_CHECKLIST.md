...
- [ ] 建立 `refactor` 分支、变更日志和 ADR 目录；记录当前 Web/Electron/Tauri/Capacitor 构建基线。
...
- [ ] 删除 `@stapxs/umami-logger-typescript`、Umami 页面和所有统计上报代码；确认无隐式 beacon、指纹、崩溃上传。
- [ ] 默认禁止向第三方请求；将地图、音乐、链接预览、更新检查等外部服务做成显式可选开关。
- [ ] 令牌使用系统密钥链或加密存储；禁止日志记录 token、完整 URL、聊天内容、用户 ID 和文件路径。
...
- [ ] 迁移 ESLint flat config，启用 TypeScript strict、noUncheckedIndexedAccess、exactOptionalPropertyTypes、useUnknownInCatchVariables。
...
- [ ] 建立 OneBot schema 包：事件/API 请求响应使用 Zod；必要时评估 protobuf 仅用于明确的高吞吐内部通道，不改变 OneBot 公共 JSON 协议。
