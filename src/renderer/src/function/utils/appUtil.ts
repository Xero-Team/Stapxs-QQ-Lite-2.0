...
export function createMenu() {
    ...
    if (backend.isDesktop()) {
        ...
        menuTitles.title = $t('Xero QQ Lite')
        menuTitles.about = $t('关于') + ' ' + $t('Xero QQ Lite')
        menuTitles.update = $t('检查更新…')
        menuTitles.hide = $t('隐藏') + ' ' + $t('Xero QQ Lite')
        menuTitles.hideOthers = $t('隐藏其他')
        menuTitles.unhide = $t('全部显示')
        menuTitles.quit = $t('退出') + ' ' + $t('Xero QQ Lite')
    ...
    backend.addListener(undefined, 'app:about', () => {
        const popInfo = {
            title: app.config.globalProperties.$t('关于') + ' ' +
                app.config.globalProperties.$t('Xero QQ Lite'),
...
export function checkOpenTimes() {
    ...
    if (times != null) {
        ...
        if (getTimes % 20 == 0) {
            ...
            html += `<span>${$t('好耶！Xero QQ Lite 已经被打开 {times} 次了！', { times: getTimes })}</span>`
